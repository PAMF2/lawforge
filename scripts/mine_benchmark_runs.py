...
def main() -> None:
    ...
    ap.add_argument("--top-models", type=int, default=5,
                    help="number of best models to harvest from (by f1)")
    ap.add_argument("--max-rows", type=int, default=2000,
                    help="cap total mined responses")
    ap.add_argument("--min-correct-streak", type=int, default=2,
    ...
    print(f"[mine-bench] loading runs (90k rows)...", file=sys.stderr)
    ...
    for row in runs:
        ...
        if out.exists():
            continue
        out.write_text(f"-- mined from {row['model_id']} on {row['problem_id']}\n"
    ...
    print(f"[mine-bench] done. saved={saved} unique problems "
          f"into {ACCEPTED}", file=sys.stderr)

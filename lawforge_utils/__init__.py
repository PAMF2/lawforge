...
def problem_hash(p: dict) -> str:
    blob = json.dumps(
        {"h": p.get("hypothesis", ""), "g": p.get("goal", "")}, sort_keys=True
    ).encode()
...
def render_prompt(eq1: str, eq2: str, *, ce_hint: str = "(unknown)") -> str:
    ...
    return _PROMPT_TPL.format(eq1=eq1, eq2=eq2, cheatsheet=_CHEATSHEET, ce_hint=ce_hint)


_PROMPT_TPL_PATH = ROOT / "solver" / "prompt_template.txt"
_CHEATSHEET_PATH = ROOT / "solver" / "cheatsheet.md"
_PROMPT_TPL = (
    _PROMPT_TPL_PATH.read_text()
    if _PROMPT_TPL_PATH.exists()
    else "Eq1: {eq1}\nEq2: {eq2}\n{cheatsheet}{ce_hint}"
)
